...
penclip plugin init @acme/hello-plugin --output ~/dev/paperclip-plugins
...
penclip plugin install ~/dev/paperclip-plugins/hello-plugin
...
Relative paths are resolved against the current working directory, so `penclip plugin install .` from inside the plugin folder works too.
...
- **npm packages are the deployable artifact.** `penclip plugin install @acme/plugin-foo` (optionally `--version 1.2.3`) installs from your configured npm registry, version-pins, and produces an install record that other operators can reproduce. Ship plugins this way.
...
- **Restart cleanly:** `penclip plugin disable <key>` pauses the plugin without removing it. `penclip plugin enable <key>` brings it back. `penclip plugin uninstall <key>` removes the install record; add `--force` to also purge plugin state and settings.
- **Browse examples:** `penclip plugin examples` lists the bundled example plugins that ship with the repo, each with a ready-to-run `penclip plugin install <path>` line.
