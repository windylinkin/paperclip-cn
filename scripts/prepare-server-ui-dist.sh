...
# This keeps @penclipai/server publish artifacts self-contained for static UI serving.
...
if [ "$should_reuse_existing_ui_dist" = true ] && [ -f "$UI_DIST/index.html" ]; then
  echo "  -> Reusing existing @penclipai/ui dist output"
else
  echo "  -> Building @penclipai/ui..."
  pnpm --dir "$REPO_ROOT" --filter @penclipai/ui build
fi
